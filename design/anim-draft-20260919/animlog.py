...
class AnimLog:
    def __init__(self, frames, start_fb=343, reserved=(0, 78, 335, 341, 342, 343)):
        self.start_fb = start_fb; self.reserved = set(reserved); self.frames = frames; self.state = 'start'; self.gates = 0; self.msgs = []; self.held = None
    ...
    def feed(self, line):
        ...
        def to(new, need_gates=0):
            ...
            self.state = new
        if s == 'start' and line == 'INHERITED_DRM_FILE planes 95/128 on FB%d, mode 340 active; frames=%d' % (self.start_fb, self.frames): to('inherited'); return 'step'
        ...
        if m and ((s == 'vk' and m.group(1) == 'A') or (s == 'bufA' and m.group(1) == 'B')):
            fb = int(m.group(2))
            if fb in self.reserved or fb in self.fb.values(): raise RuntimeError('bad fb ' + line)
            self.fb[m.group(1)] = fb; to('buf' + m.group(1), 1); return 'buffer'
        if s == 'bufB' and line == 'TEST_ONLY_PASS A B FB343': to('tested', 1); return 'step'
        ...
        if s == 'summary' and line == 'RETURNED_FB343': to('returned', 2); return 'returned'
        if s == 'returned' and line == 'VK_DESTROYED': to('destroyed', 2); return 'destroyed'
        if s == 'destroyed' and line.startswith('HELD ') and line.endswith('reason=ANIMATION_DONE_FB343_RETAINED'):
