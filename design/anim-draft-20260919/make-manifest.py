#!/usr/bin/env python3
"""Build the finite GPU animation bundle. Requires the visually confirmed render bundle. Output: ~/y700-agent/vkanim<frames>-<hash16>/."""
...
R2 = AGENT/'vkrender-a9156afd13ae757f'
A1 = AGENT/'vkanim120-8b87d4cb2d247137'
FRAMES = int(sys.argv[1]) if len(sys.argv) > 1 else 0
...
FILES = ['run-anim.py', 'vkanim.py', 'vkabi.py', 'drmabi.py', 'animpattern.py', 'animlog.py', 'borrow.py', 'anim.spv', 'anim.spvasm', 'gpuguard.py', 'health_guard.py', 'runtime-readers.py', 'y700lib.py', 'expected-state.txt']
...
need(FRAMES in (120, 600), 'usage: make-manifest.py 120|600')
sr = json.loads((R2/'state-review.json').read_text()); w4 = json.loads((R2/'worker.json').read_text())
need(sr['corrected_matcher_accepts'] and (R2/'render-visual.json').is_file(), 'render bundle not confirmed')
need(sha(B/'expected-state.txt') == sha(R2/'state-now.txt') == sr['state_now_sha256'], 'expected state is not the reviewed FB343 text')
ra = json.loads((A1/'result.json').read_text()); w5 = json.loads((A1/'worker.json').read_text())
need(ra.get('status') == 'STOPPED_INSPECT_RETAINED' and "unexpected DRM event (2, 32, 0, 205)" in ra.get('error', ''), 'previous animation state unexpected')
m['retained_workers'] = s2['retained_workers'] + [{'pid': x['pid'], 'starttime': x['starttime']} for x in (w, w1, w2, w3, wf, w4, w5)]
...
m['anim_review'] = {'frames': FRAMES, 'base_fb': 343, 'base_state_sha256': sha(B/'expected-state.txt'), 'start_fb': 345,
                    'reserved_fbs': [0, 78, 335, 341, 342, 343, 344, 345],
                    'start_note': 'vkanim120-8b87d4cb2d247137 committed frame 1 to FB345 (user saw the small ring) then stopped: drmabi.atomic sent user_data 0; fixed'}
for key in ('anim_review', 'vk_review', 'retained_workers', 'original_worker', 'original_executable', 'files'):
    need(key in m, 'manifest missing top-level key: '+key)
need(m['anim_review']['frames'] == FRAMES, 'frames mismatch')
text = json.dumps(m, indent=2).encode()
out = AGENT/('vkanim%d-' % FRAMES+hashlib.sha256(text).hexdigest()[:16])
