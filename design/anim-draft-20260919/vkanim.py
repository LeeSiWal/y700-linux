...
render buffer, two NEW cached dumb buffers A/B + FBs on the inherited DRM file, TEST_ONLY for A, B and FB343. Gate 2: for k=1..N:
GPU renders frame k (fence), CPU checks ~1.2k pixels, copies into the back buffer (never the one on screen), one blocking atomic
FB_ID switch of planes 95+128 with PAGE_FLIP_EVENT (consumed before the next frame). Finally returns to the confirmed FB343,
destroys Vulkan (kgsl fd closes; PID13398 keeps its own) and holds all DRM resources. Any error: HOLD, no retry."""
...
SIZE = PITCH * H
BASE_FB = 343
START_FB = int(os.environ.get('ANIM_START_FB', '0'))
RESERVED_FBS = {int(x) for x in os.environ.get('ANIM_RESERVED_FBS', '').split(',') if x}
...
def main():
    ...
    if not (crtc['ACTIVE'][1] == 1 and crtc['MODE_ID'][1] == 340): out('STOP: display state'); hold('STATE_CHANGED')
    if START_FB not in (343, 345) or not {0, 78, 335, 341, 342, 343, 344, 345} <= RESERVED_FBS: out('STOP: start/reserved fbs'); hold('USAGE')
    geometry(p, START_FB)
    fbprop = {95: p['left']['FB_ID'][0], 128: p['right']['FB_ID'][0]}
    out('INHERITED_DRM_FILE planes 95/128 on FB%d, mode 340 active; frames=%d' % (START_FB, FRAMES))
    ...
    for label in ('A', 'B'):
        ...
        if not (g[1:4] == (W, H, d.XRGB8888) and g[9] == PITCH and g[13] == 0 and g[17] == 0 and fb not in RESERVED_FBS):
    ...
    hold('ANIMATION_DONE_FB343_RETAINED')
