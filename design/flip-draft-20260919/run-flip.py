#!/usr/bin/env python3
"""DRAFT supervisor: finite A/B flip on the confirmed two-plane native layout, then back to FB335."""
...
def check_state(label):
    # Initial, after-TEST_ONLY and final states must all equal the user-confirmed native state.
...
def worker_identity(child,identity,m):
    ...
    need((p/'exe').resolve()==B/'flip-held','worker executable changed')
    need(hashlib.sha256(c.read(p/'exe',True)).hexdigest()==m['files']['flip-held'],'worker binary changed')
...
def supervise(child,identity,m,monitor,report,base):
    log=FlipLog();start=time.monotonic();ready=None;go=None;returned=None;offset=0;pending=b''
    try:
        while True:
            ...
            while b'\n' in pending:
                ...
                if event=='done':
                    ...
                    report['flips'].append(dict(log.done[-1],underrun=counts))
                    if counts!=base:
                        stop_worker(child,report,'underrun counter changed')
                        need(False,'underrun counter changed during flips: '+str(base)+' -> '+str(counts))
                if event=='returned':
                    returned=now;report['flips_returned']=True
                    report['snapshots'].append(snapshot('immediately-after-flips'))
                    check_state('after-flips')
            if ready is not None and go is None and now-ready>=2:
                ...
                report['snapshots'].append(snapshot('before-flips'))
                save('commit-intent.json',{'boot_id':m['boot_id'],'worker':identity,'time':time.time()})
                ...
                print('FLIP_GATE_RELEASED',flush=True)
                child.stdin.write(b'GO\n');child.stdin.flush()
            if go is not None and returned is None and now-go>=STEPS*0.5+10:
                stop_worker(child,report,'flip deadline')
                need(False,'flips did not finish in time; worker asked to hold')
            if returned is not None and now-returned>=30:
                ...
                need(underruns(c.read('/sys/kernel/debug/dri/0/encoder68/status'))==base,'underrun changed after flips')
                report['snapshots'].append(snapshot('after-30-seconds'))
                report['status']='FLIPS_DONE_FB335_HELD_AWAITING_VISUAL_CONFIRMATION'
                print(report['status'],flush=True)
                return
            need(now-start<120,'flip trial timeout; resources retained, no retry')
            time.sleep(.25)
    finally:
...
def main():
    ...
    with (B/'flip.lock').open('a') as lock:
        ...
        need(base=={int(k):v for k,v in m['flip_review']['underrun_base'].items()},'underrun base differs from review')
        save('attempt.json',{'boot_id':m['boot_id'],'time':time.time()})
        report={'boot_id':m['boot_id'],'gate_released':False,'flips_returned':False,'flips':[],
                'underrun_base':base,'dmesg_before':c.dmesg(),'snapshots':[snapshot('before-worker')],
                'physical_output':'requires user observation; events/readback are not proof'}
        try:
            borrowed=borrow_fd(m,c)
            try:
                with (B/'worker.log').open('xb',buffering=0) as out:
                    child=subprocess.Popen([str(B/'flip-held'),'--inherited-fd',str(borrowed)],pass_fds=(borrowed,),stdin=subprocess.PIPE,stdout=out,stderr=subprocess.STDOUT,start_new_session=True)
            finally:
            ...
            identity={'pid':child.pid,'starttime':c.task_stat(c.read(Path('/proc')/str(child.pid)/'stat'))['starttime'],
                      'boot_id':m['boot_id'],'binary_sha256':m['files']['flip-held']}
