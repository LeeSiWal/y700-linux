#!/usr/bin/env python3
"""One-shot KGSL stage 2: create /dev/kgsl-3d0 (495:0, 0600) and let a held worker open it once.
First open boots GMU and loads SQE/AQE/zap (TZ). Only DEVICE_INFO is queried; no submission.
The fd is never closed; nothing is unloaded; display, retained workers and USB Ethernet are checked every tick."""
from pathlib import Path
import fcntl, hashlib, json, os, stat, subprocess, sys, time, signal
import health_guard as h
import gpuguard as g
from openlog import OpenLog
...
def worker_ok(child, identity):
    ...
    need(c.read(p/'cmdline', True).split(b'\0')[:3] == [b'python3', b'-B', str(B/'kgsl-open.py').encode()], 'worker command changed')

def make_node():
    need(c.read('/sys/class/kgsl/kgsl-3d0/dev').strip() == '495:0', 'kgsl device number changed')
    need(not NODE.exists() and not NODE.is_symlink(), '/dev/kgsl-3d0 already exists')
    os.mknod(NODE, 0o600 | stat.S_IFCHR, os.makedev(495, 0))
    st = os.lstat(NODE)
    need(stat.S_ISCHR(st.st_mode) and st.st_rdev == os.makedev(495, 0) and stat.S_IMODE(st.st_mode) == 0o600 and st.st_uid == 0, 'node readback mismatch')

def supervise(child, identity, monitor, report):
    log = OpenLog(); start = time.monotonic(); ready = go = held = None; offset = 0; pending = b''
    try:
        while True:
            ...
            while b'\n' in pending:
                ...
                if ev == 'devinfo': report['devinfo'] = log.devinfo; report['open_ms'] = log.open_ms
                if ev == 'held': held = now; monitor.phase = 'held observation'; report['snapshots'].append(snap('after-open'))
            if ready is not None and go is None and now-ready >= 2:
                ...
                save('open-intent.json', {'boot_id': monitor.m['boot_id'], 'worker': identity, 'time': time.time()})
                go = now; log.go_sent = True; monitor.phase = 'first open'; report['gate_released'] = True
                print('OPEN_GATE_RELEASED', flush=True)
                child.stdin.write(b'GO\n'); child.stdin.flush()
            if go is not None and held is None: need(now-go < 30, 'first open/devinfo did not finish in 30s; worker retained')
            if held is not None and now-held >= 60:
                monitor.observe(2, quiet=True); worker_ok(child, identity)
                report['snapshots'].append(snap('after-60-seconds'))
                report['status'] = 'KGSL_OPENED_DEVINFO_OK_FD_HELD'
                print(report['status'], json.dumps(log.devinfo), flush=True)
                return
...
def main():
    ...
    need(gpu_view() == m['gpu_review']['expected_before'], 'GPU state differs from review: '+json.dumps(gpu_view()))
    for w in m['retained_workers']:
        p = Path('/proc')/str(w['pid'])
        need(p.exists() and c.task_stat(c.read(p/'stat'))['starttime'] == w['starttime'], 'retained DRM worker changed: '+str(w['pid']))
    print('PREFLIGHT PASS: same boot, 167 module identities, KGSL bound, no /dev node, firmware hashes, 13 display workers.', flush=True)
    ...
    with (B/'apply.lock').open('a') as lock:
        ...
        try:
            make_node(); report['node'] = '/dev/kgsl-3d0 c 495:0 0600 root'; print('NODE_CREATED /dev/kgsl-3d0 495:0 0600', flush=True)
            monitor.observe(3, quiet=True)
            with (B/'worker.log').open('xb', buffering=0) as out:
                child = subprocess.Popen(['python3', '-B', str(B/'kgsl-open.py')], stdin=subprocess.PIPE, stdout=out,
