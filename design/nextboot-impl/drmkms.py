...
GETCONNECTOR = struct.Struct('<QQQQ12I')        # encoders modes props prop_values ptrs | count_modes count_props count_encoders
                                                # encoder_id connector_id connector_type connector_type_id connection mm_w mm_h subpixel pad
IOCTL['GETCONNECTOR'] = iowr(0xA7, GETCONNECTOR.size)

def connector_encoder(fd, connector_id):
    """Current encoder id of a connector (counts-only query: no arrays are copied)."""
    r = ioctl(fd, 'GETCONNECTOR', GETCONNECTOR, 0, 0, 0, 0, 0, 0, 0, 0, connector_id, 0, 0, 0, 0, 0, 0, 0)
    if r[8] != connector_id: raise RuntimeError('connector id mismatch')
    return r[7]
