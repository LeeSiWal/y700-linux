...
def good():
    return ['READY_FOR_PROBE', 'ICD_LOADED', 'NEGOTIATED interface=5', 'MESA: info: Created an instance', 'INSTANCE_CREATED',
            'ENUMERATE_ENTER', 'MESA: warning: Unable to open neither /dev/dma_heap/system nor /dev/ion', 'PHYSICAL_DEVICES count=1',
...
class T(unittest.TestCase):
    ...
    def test_good(self):
        log = run(good()); self.assertEqual(log.props['deviceName'], 'Adreno (TM) 840'); self.assertEqual(len(log.mesa), 2)
    def bad(self, i, new):
        l = good(); l[i] = new
    ...
    def test_two_devices(self): self.bad(7, 'PHYSICAL_DEVICES count=2')
    def test_wrong_device(self): self.bad(8, 'PROPERTIES ' + json.dumps(dict(P, deviceName='llvmpipe', vendorID=0x10005)))
    def test_early_hold(self): self.bad(5, 'HELD pid=1 reason=INSTANCE_FAILED')
    def test_out_of_order(self):
        l = good(); l[4], l[5] = l[5], l[4]
        with self.assertRaises(RuntimeError): run(l)
